...
{
    /// <summary>
    /// Redis 引用需在Webconfig 中增加ServerHosts
    /// 如  <add key="ServerHosts" value="192.168.155.2:6379"/>
    /// </summary>
    public class RedisHelper
    {
        private static string[] ReadWriteHosts = ConfigurationManager.AppSettings["ServerHosts"].Split(new char[] { ';' });
        ...
        public static void List_Add<T>(string key, List<T> lt)
        {
            using (IRedisClient redis = prcm.GetClient())
            {
                var redisTypedClient = redis.GetTypedClient<T>();
