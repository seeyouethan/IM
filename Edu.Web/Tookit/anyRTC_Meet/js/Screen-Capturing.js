...
function onMessageCallback(data) {
    ...
    if (data == 'PermissionDeniedError') {
        // chromeMediaSource = 'PermissionDeniedError';//解决第一次取消分享后会不能再次分享
    ...
    if (data == 'EndDesktop') {
        // chromeMediaSource = 'desktop';
        screenStream && screenStream.getTracks().forEach(function(track) {
          track.stop();
        });
    }
...
function stopCapturerScreen () {
    window.postMessage('cancel-desktop', '*');
}
...
function getChromeExtensionStatus(extensionid, callback) {
    ...
    if (arguments.length != 2) {
        ...
        extensionid = 'daiabbkkhgegdmhfpocaakcgbajnkgbp'; // default extension-id
    }
...
function getScreentStream (callback) {
    if (isFirefox) {
...
function getScreenConstraints(callback) {
    // console.warn('This deprecated API should no longer be used, but will probably still work.')
