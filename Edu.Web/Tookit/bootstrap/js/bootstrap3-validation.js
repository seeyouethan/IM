...
!function($) {
   ...
   $.fn.validation.defaults = {
       validRules: [
             { name: 'userName', validate: function (value) { return (!/^[a-zA-Z0-9_]{5,16}$/.test(value)); }, defaultMsg: '输入不合法。' },
             ...
             { name: 'pin', validate: function (value) { return (!/^(\d{6})(18|19|20)?(\d{2})([01]\d)([0123]\d)(\d{3})(\d|X)?$/.test(value)); }, defaultMsg: '非法' },
             { name: 'phone', validate: function (value) { return (!/^((\(\d{2,3}\))|(\d{3}\-?))?(\(0\d{2,3}\)|0\d{2,3}-?)?[1-9]\d{6,7}(\-\d{1,4})?$/.test(value)); }, defaultMsg: '非法' },
            { name: 'required', validate: function (value) { return ($.trim(value) == ''); }, defaultMsg: '*' },
             { name: 'noEmpty', validate: function (value) { return ($.trim(value) == ''); }, defaultMsg: '' },
            ...
            { name: 'date', validate: function (value) { return (/Invalid|NaN/.test(new Date(value).toString())); }, defaultMsg: "日期格式XXXX-XX-XX。" }, { name: 'RePwd', validate: function (value) { return (value != $('#Pwd').val()); }, defaultMsg: '两次输入密码不一致。' }
        ],
        reqmark:true,
